...
<html>
<head>
<title><?= $display_name ?> - NIK System</title>
...
<div class="top-bar">
    ...
    <!-- NEW CLEAR BUTTON -->
     <button id="scrollTopBtn" onclick="scrollToBottom()" style="position: fixed; bottom: 20px; left: 20px; padding: 12px 14px; border: none; background: #fff; color: #000000; border-radius: 50%; cursor: pointer; box-shadow: 0 2px 8px rgba(0,0,0,0.2);">
    🔻
    </button>
    <div class="btn-group">
...
$query = "SELECT * FROM $table 
         ORDER BY 
    (disabled_until IS NOT NULL AND disabled_until > NOW()) ASC,
    total_use ASC,
    shuffle_order ASC
";
...
<div style="text-align:center;">
    <a href="add-data.php?table=<?= $table ?>">
...
<button id="scrollTopBtn" onclick="scrollToTop()" style="position: fixed; bottom: 20px; right: 20px; padding: 12px 14px; border: none; background: #fff; color: #000000; border-radius: 50%; cursor: pointer; box-shadow: 0 2px 8px rgba(0,0,0,0.2);">
    ↑
</button>
...
function restoreNIK(el, id, table){

    const row = el.closest("tr");
    const tbody = row.parentElement;

    fetch(`restore.php?id=${id}&table=${table}`)
    .then(res => res.json())
    .then(data => {
        ...
        if(data.success){
            ...
            // change action column back to Copy button
...
window.addEventListener("load", () => {
    ...
    showToast("<?= addslashes($_SESSION['toast']) ?>");
